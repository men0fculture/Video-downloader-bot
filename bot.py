...
def download_video(url):
    ydl_opts = {
        # 1080p selection with mp4 preference
        'format': 'bestvideo[height<=1080][ext=mp4]+bestaudio[ext=m4a]/best[height<=1080][ext=mp4]/best',
        'outtmpl': f'{DOWNLOAD_DIR}/%(title)s.%(ext)s',
        'quiet': True,
        'merge_output_format': 'mp4',
        'noplaylist': True,
        # No-Cookie & Bot Detection Bypass
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        # Ensure we get the merged mp4 file path
...
def process_and_split(input_path, clip_duration=30):
    ...
    for i, start in enumerate(range(0, int(total_seconds), clip_duration)):
        ...
        # Re-encoding is MUST for playability and removing errors
        # No complex filters, just standard conversion
        cmd = [
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🎬 Bot Ready! Send me a YouTube link.")
