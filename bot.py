...
def download_video(url):
    # Try 1080p first (prefer mp4 with audio), then fallback to best
    ydl_opts = {
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "• Download in **highest quality (prefer 1080p)**\n"
...
async def handle_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status = await update.message.reply_text("📥 Downloading video (prefer 1080p)...")
    try:
        ...
        await status.edit_text(f"❌ Download error: {str(e)[:200]}")
        return
