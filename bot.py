...
def download_video(url):
    """
    Download video with best quality, preferring 1080p if available.
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        # Get actual height from formats
        formats = info.get('formats', [])
        height = None
        for f in formats:
            if f.get('format_id') == info.get('format_id') and f.get('height'):
                height = f['height']
                break
        if height:
            print(f"Downloaded resolution: {height}p")
        return filename, title, description, uploader, tags, views, likes, duration, height
...
def generate_caption(title, description, uploader, tags, views, likes, part, total, height=None):
    hashtags = []
    if tags:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "🎬 **Video Clipper Bot (Auto Best Quality)**\n\n"
        "Send any YouTube link. I'll:\n"
        "• Download in **highest available quality (prefer 1080p)**\n"
...
async def handle_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status = await update.message.reply_text("📥 Downloading video (best quality, prefer 1080p)...")
    try:
        video_path, title, desc, uploader, tags, views, likes, duration, height = download_video(url)
    ...
    total = len(clips)
    quality_msg = f"{height}p" if height else "best available"
    await status.edit_text(f"✅ Downloaded in {quality_msg}\n🎬 {total} clips. Sending...")

    for i, clip in enumerate(clips):
        caption = generate_caption(title, desc, uploader, tags, views, likes, i+1, total, height)
...
def main():
    ...
    print("🤖 Auto Best Quality Video Clipper Bot is running...")
    app.run_polling()
