...
def file_is_valid_video(path):
    """Check if file exists and has video stream."""
...
def download_video(url):
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        # if filename ends with .webm or .mkv, change to .mp4
        base, ext = os.path.splitext(filename)
        if ext not in ['.mp4']:
            possible_mp4 = base + '.mp4'
            if os.path.exists(possible_mp4):
                filename = possible_mp4
        if not file_is_valid_video(filename):
            raise Exception(f"Downloaded file is not valid: {filename}")
...
def download_via_invidious(url):
    ...
    try:
        ...
        if not file_is_valid_video(filename):
            raise Exception("Downloaded file not valid")
...
def make_clips(input_path, clip_duration=30):
    ...
    for i, start in enumerate(range(0, int(duration), clip_duration)):
        ...
        if rc == 0 and file_is_valid_video(clip_path):
            clips.append(clip_path)
        else:
            print(f"Clip {i} failed: {err}")
...
def process_clip(input_path, output_path, title):
    ...
    rc, _, err = run_cmd(cmd, check=False)
    if rc != 0 or not file_is_valid_video(output_path):
        # fallback: without subtitles
        if srt_path:
            print("Subtitle filter failed, retrying without subtitles")
            filter_str = (
                f"setpts={1/speed}*PTS,"
                f"eq=brightness={bright}:contrast={cont},"
                f"scale=1080:1920:force_original_aspect_ratio=decrease,"
            ...
            rc, _, err = run_cmd(cmd, check=False)
            if rc != 0:
                print(f"Final fallback failed: {err}")
                return None
        else:
            return None
    # cleanup
...
async def handle_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not video_path or not file_is_valid_video(video_path):
    ...
    for i, clip in enumerate(clips):
        ...
        if i > 0:
            avg_time = elapsed / i
            remaining = avg_time * (total_clips - i)
        ...
        result = process_clip(clip, final_path, title)
        if not result or not file_is_valid_video(final_path):
            await update.message.reply_text(f"⚠️ Clip {i+1} processing failed, skipping.")
            os.remove(clip)
            continue
