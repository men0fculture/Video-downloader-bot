...
def get_average_brightness(video_path):
...
def process_clip(input_path, output_path):
    # Brightness correction (safe)
    brightness_val = get_average_brightness(input_path)
    if brightness_val is None:
        bright, cont = 1.0, 1.0
    elif brightness_val < 100:
        bright, cont = 1.05, 1.02
    elif brightness_val > 150:
        bright, cont = 0.95, 0.98
    else:
        bright, cont = 1.0, 1.0
    ...
    filter_str = f"eq=brightness={bright}:contrast={cont}"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "• Adjust brightness/contrast (safe)\n"
        "• Add slight speed variation\n"
        "• Send clips with metadata & hashtags\n\n"
        "✅ No portrait, no subtitles – simple and stable."
    )
...
async def handle_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    total = len(clips)
    await status.edit_text(f"🎬 {total} clips. Processing (brightness + speed)...")
    ...
    for i, clip in enumerate(clips):
        ...
        success = process_clip(clip, final_path)
        ...
        if success and is_valid_video(final_path):
            ...
            # fallback: send raw clip
            caption = generate_caption(title, desc, uploader, tags, views, likes, i+1, total) + "\n\n⚠️ Original clip (processing failed)"
...
def main():
    ...
    print("🤖 Simple Video Bot (no portrait) is running...")
    app.run_polling()
