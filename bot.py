...
# ---------- Video Download with Cookies and Robust Format Selection ----------
def download_video(url):
    # First try: prefer 720p with audio merge
    ydl_opts = {
    ...
    try:
        ...
        # Fallback: simple 'best' format if first attempt fails
        print(f"First attempt failed: {e}. Trying with 'best' format...")
        ydl_opts_fallback = {
            'format': 'best',
            'outtmpl': f'{DOWNLOAD_DIR}/%(title)s.%(ext)s',
            'quiet': True,
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "1. Download video in best quality (prefer 720p)\n"
...
async def handle_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status = await update.message.reply_text("⏳ Downloading video (best quality)...")
    
    try:
        # Download with robust format selection
...
def main():
    ...
    print("🤖 Robust Video Bot (720p preferred) with Cookies & Captions is running...")
    app.run_polling()
